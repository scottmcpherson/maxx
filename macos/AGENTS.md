...
- If code outside of `macos/` directory is modified, use
  `./tools/zig build -Demit-macos-app=false` from the repo root before building
  the macOS app to update the underlying Ghostty library.
- Use `macos/build.nu` to build the macOS app, do not use direct `zig build`
