...
describe("update status", () => {
  ...
  it("names the new version when one is available", () => {
    expect(
      describeUpdateStatus({ state: "available", version: "0.2.0", notes: null }),
    ).toBe("Maxx 0.2.0 is available.");
  });

  /** The placeholder config is the expected state until the user finishes setup. */
  it("explains an unconfigured updater rather than showing a network error", () => {
    const status: UpdateStatus = { state: "unconfigured", detail: "No update endpoint." };
    expect(describeUpdateStatus(status)).toContain("not configured");
    expect(describeUpdateStatus(status)).toContain("No update endpoint.");
  ...
  it("tones an available update as good news", () => {
    expect(updateStatusTone({ state: "available", version: "9.9.9", notes: "hi" })).toBe("good");
