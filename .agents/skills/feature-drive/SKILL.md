---
name: feature-drive
description: Pick up a Maxx Linear issue (team Maxx, MAX-NNN) and drive it to done — fetch the issue, work in a dedicated git worktree, implement, build and exercise the running app with computer use + screenshots, run /code-review, attach evidence, and move the Linear issue through its statuses. Use when the user points at a Linear issue ID or URL ("/feature-drive MAX-123", "drive MAX-12", "pick up the next issue"), asks what to work on next from Linear, or hands off an issue for implementation.
---
...
2. **Build and drive the app.** From the worktree, create a dev build and launch it with `zig build run`, then use **computer use** to exercise the change in the running app — click/type through the relevant flow and take **screenshots** along the way. Computer use drives whatever is frontmost, so when several drives run in parallel, verify one build at a time. Every acceptance criterion must be observably met in the screenshots; a criterion with no supporting screenshot gets called out in the report, never assumed.
