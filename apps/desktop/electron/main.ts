...
const projectDirectory = app.isPackaged ? desktopDirectory : path.resolve(desktopDirectory, "../..");
const development = process.argv.includes("--dev");
const checkoutBuild = process.argv.includes("--checkout-build");
const bundleID = process.argv.find((argument) => argument.startsWith("--bundle-id="))?.slice("--bundle-id=".length)
  || (checkoutBuild ? "com.maxx.preview" : "com.maxx.app");
const developmentLabel = process.env.MAXX_DEV_LABEL?.trim() || "main";
const developmentRendererURL = configuredDevelopmentRendererURL();
...
function configuredDevelopmentRendererURL(): string {
  const value = process.env.MAXX_RENDERER_URL || "http://localhost:1420";
  const url = new URL(value);
  if (url.protocol !== "http:" || url.hostname !== "localhost" || url.username || url.password) {
    throw new Error(`Invalid Maxx development renderer URL: ${value}`);
  }
  return url.origin;
}
...
function isDevelopmentRendererURL(value: string): boolean {
  try {
    return new URL(value).origin === developmentRendererURL;
  } catch {
    return false;
  }
}
...
function runtimeExecutable(): string {
  ...
  return path.join(desktopDirectory, "src-tauri", "target", "debug", "maxx");
}
...
function trayIconPath(): string {
  return app.isPackaged
    ...
    : path.join(desktopDirectory, "src-tauri", "icons", "tray.png");
}
...
function installTray(): void {
  ...
  tray = new Tray(icon);
  tray.setToolTip(development ? `Maxx - ${developmentLabel}` : checkoutBuild ? app.getName() : "Maxx");
...
async function createWindow(): Promise<void> {
  ...
  mainWindow = new BrowserWindow({
    ...
    minWidth: 900,
    minHeight: 600,
    title: development ? `Maxx - ${developmentLabel}` : checkoutBuild ? app.getName() : "Maxx",
  ...
  mainWindow.webContents.on("will-navigate", (event, url) => {
    const allowed = development ? isDevelopmentRendererURL(url) : url.startsWith("file:");
  ...
  if (!existsSync(executable)) throw new Error(`Maxx runtime is missing at ${executable}; build it with cargo build --manifest-path apps/desktop/src-tauri/Cargo.toml`);
  computerUseHost ??= new ComputerUseHost(
    computerUseBinaryPath(desktopDirectory, app.isPackaged),
    bundleID,
    development,
  );
  ...
  if (development) await mainWindow.loadURL(developmentRendererURL);
  else await mainWindow.loadFile(path.join(desktopDirectory, "dist", "index.html"));
}
...
if (!smokeMode && !app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(async () => {
    ...
    if (process.platform === "darwin" && !app.isPackaged) app.dock?.setIcon(nativeImage.createFromPath(path.join(desktopDirectory, "src-tauri", "icons", "128x128.png")));
    await createWindow();
    if (terminalUiSmoke) {
